...
SELECT `news`.`title`, `news`.`date_created`, `user`.`login`, `user`.`thename`, `category`.`title`, `category`.`slug`
	FROM `news`
	INNER JOIN `user`
		ON `news`.`user_iduser` = `user`.`iduser`
...
SELECT `news`.`title`, `news`.`date_created`, `user`.`login`, `user`.`thename`, `category`.`title` AS categ_title, `category`.`slug`
	FROM `news`
	INNER JOIN `user`
		ON `news`.`user_iduser` = `user`.`iduser`
	LEFT JOIN `news_has_category`
...
# on sélectionne les champs title, date_created de la table news ainsi que le login et thename de la table user, uniquement lorsque une news a un utilisateur. On récupère également les category.idcategory category.title et category.slug Si la news a des catégories (on affiche la news Si elle n'a pas de catégorie avec ALIAS de sortie pour éviter les doublons sur title. On va créer des alias internes, ce sont des alias pour le nom des tables, pas pour sortie. On va grouper par la clef primaire ou l'* de news)
...
>>>>>>> 316618abcfdf8708ece9a2c5042a240f3f1299f0
	FROM news n
	LEFT JOIN `user` u
		ON n.user_iduser = u.iduser
	LEFT JOIN news_has_category h
